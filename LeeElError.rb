# Lee El Error
# Como lo hemos dicho tantas veces al escribir código es muy importante refactorizarlo para que este organizado y 
# sea fácil de leer. Encadenar métodos es un técnica que busca crear código más claro y conciso.
# En un principio será coherente escribir código seguido y en varias lineas, donde verticalmente se comprenda el 
# flujo del programa pero a la larga no es lo más eficiente.
# Al refactorizar hay que tratar de usar las menos líneas posibles, pero hay que ser inteligentes y saber en 
# que momento, menos líneas, ponen en riesgo la facilidad de lectura y es ahí donde hay que parar.
# En Ruby el código que se lee más fácil y está elegante organizado es más profesional que el compacto y complejo.
# Si entiendes como funcionan las Clases y sus Métodos puedes leer este artículo para comprender como se escribe 
# código en el que los métodos podrán ser encadenados:
...
# El segundo de los métodos es un ejemplo de un método de una linea pero muy complicado de leer y entender que 
# hace, trata de mejorarlo aunque uses más lineas.
...
def max_letter_frequency_per_word(sentence)
    ...
    array_letter = words_longer_than(array,3)
    num_array = letter_per_word(array_letter, "e")
    return numbers_larger_than(num_array)
end
